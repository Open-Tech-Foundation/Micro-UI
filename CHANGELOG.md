...
- Refined the marketing language to distinguish the small runtime from its micro apps, and changed the browser favicon to the official OTF logo.
- Updated `website/package.json` so the default `build` script produces the static site with `otfw build --ssg`.
...
- The website showcase now statically imports Micro-UI so production bundles cannot leave an unresolved bare package specifier in the browser.
...
### Fixed
- **test/browser/run.mjs** — the headless browser's profile goes to the OS temp directory instead of `test/browser/.chrome-profile`. Chrome leaves broken symlinks in a profile (`SingletonLock`, `SingletonCookie`), Biome walks the tree and warns about each one, and `tsr lint` reported two warnings on every run — which is how a project teaches people to ignore its own warnings. Nothing is written into the working tree by a browser run now. The `.gitignore` entry stays, so anyone with a profile left over from before is still covered.
- **demo** — the workbench mounts with `{ dev: true }`, so it shows a failing component's real message in the page and reports the two silent mistakes dev mode now catches. It is a development workbench that was hiding its own diagnostics. Every page was visited twice with it on: the console stays clean, so nothing in the demo was leaking a subscription or handing `class` an array.
- **test.html** — the browser suite covers the user-facing fixes from this run: rows interpolated into a `<tbody>` (foster parenting is a real-browser behaviour, so it is pinned where it actually happens, reorder included), a `<select>` taking its bound value on first render and then not overriding a choice the user made, the caret staying put when a field's value is rewritten, and a `javascript:` URL being refused — with a click on the refused link to prove nothing runs. 43 assertions to 60. These behaviours were verified in Chromium as they were written but the verification was throwaway; `AGENTS.md` asks for an E2E test when the change is user-facing, and these are those tests.
- **test/jsdom/template.adversarial.test.mjs** — "static on* attribute without MARKER throws actionable error" mounts with `{ dev: true }` instead of assuming it. The assertion reads the error *message* out of the page, which only dev mode puts there, so the test was relying on another file in the run having switched dev mode on: it passed in the full suite and failed whenever its own file was run alone. Its other half is now a test of its own — the same failure with dev off says only "Something went wrong.", which is the contract that keeps a thrown message's URLs and tokens off the page. Every other test file already passes standalone; this was the only one.
- **error.ts** — the error box carries `role="alert"`. It replaces whatever the user was looking at, so without it a component failing is silent to a screen reader: the content is simply gone.
- **dom.ts** — an `href`, `src`, `action`, `formaction`, `ping`, `srcdoc` or `xlink:href` whose value would run script is refused rather than written. `href=${untrusted}` was set verbatim, on the reasoning that the value was never *parsed* as markup — true, and beside the point: the browser executes a `javascript:` URL when it follows one, so a URL out of a database ran on the next click with the page's origin, in a library whose headline is "secure by default". The scheme is read the way the parser reads it, with control characters and whitespace stripped, so `JaVaScRiPt:`, `java\tscript:` and a leading space are all caught; `mailto:`, `tel:`, `blob:`, `data:` images and any path that merely *contains* the word are untouched. Refused rather than blanked: a half-written URL still navigates. **This changes existing behaviour** — the adversarial test that pinned the old passthrough now pins the refusal.
- **dom.ts / vdom.ts / reconcile.ts** — a `<select value=${x}>` selects the option it was told to. Attributes were applied before children, and a select's value is one of its options, so setting it while there are none selected nothing and left the first option showing — wrong exactly when the initial selection is not the first one, and self-correcting only if the binding later changed. The value is re-applied once the children exist (`<textarea>` too, whose children are also its value), and again after the options are patched, because patching can move the selection on its own: the option holding it is removed, or an unkeyed option's own value changes underneath it. A select whose value no longer matches the binding *before* that patch was chosen by the user, and is left alone the way a typed-in input is.
- **dom.ts** — writing a bound value no longer throws the caret to the end of the field. Assigning `.value` collapses the selection, which is invisible while a binding only echoes what was typed — the strings match — but any input that *rewrites* what it is given (an uppercase field, a currency or phone mask, a trim) produced a different string on every keystroke and sent the caret to the end of each one. The write is skipped when the DOM already holds that string, and when the field is focused the selection is restored, clamped to the new length. An input type with no selection to save (`number`, `email`) is unaffected.
- **template.ts** — an interpolation inside a table renders where it was written. The binding placeholder was spliced into the markup as a text character, and the HTML parser foster-parents stray text out of `<table>`, `<tbody>`, `<thead>`, `<tfoot>`, `<tr>` and `<colgroup>` — so `` html`<tbody>${rows.map(...)}</tbody>` `` put every row *above* the table and left the tbody empty. It failed silently: no throw, nothing in the console, just a table with no rows in it. The placeholder is now a comment (`<!--\ue000-->`) in child positions, which the parser leaves in place. Attribute values keep the bare marker, and so do raw-text elements — `<textarea>`, `<title>`, `<style>`, `<script>` and friends, where `<!--` is four literal characters rather than a comment, and where a comment placeholder would have been swallowed as text and thrown every later binding onto the wrong value. `buildTemplate` tracks just enough tokenizer state (in a tag, in a quoted attribute value, in a raw-text element, in an author's comment) to tell those positions apart; it runs once per distinct template, which is already cached on the literal. Costs 403 bytes gzipped.
- **store.ts** — a listener that throws is reported instead of swallowed. `notify` caught and discarded, so a subscriber with a bug simply stopped updating its component, with nothing in the console to say why. The other listeners still run, as before.
...
### Documentation
- "Tiny" is now "small", in the tagline, the one-line description and the npm `description` field. The core is 7.1 KB gzipped and the whole package with the store is 7.8 KB, heading toward 10 KB as the silent-failure fixes land — "tiny" was a claim the numbers were quietly outgrowing. The `tiny` npm keyword is left alone: it is a search term, not a claim, and the library is still in that bucket.
- README's size claim is now split the way the package is: ~7.1 KB gzipped for the core, ~7.8 KB with the `store`. Nothing in the core imports the store, so a tree-shaking build that never touches it never pays for it, and one number hid that. (Was ~6.3 KB before the table-binding fix and these additions; the Development section says how to measure both.)
- README no longer says a cache-busting query gives each test file its own module instance. Under the previous test runner every `src/index.ts?whatever` resolved to the same module, so `devMode` and everything else at module scope was shared by the whole run — which is exactly how one test came to depend on the order files happened to run in. The Development section now says to set `{ dev: true }` or `{ dev: false }` explicitly rather than assuming the default.
- README says what "a move or a re-parent does not count" actually means: the element has to be back in the document by the next microtask. A synchronous `appendChild` keeps the component; an `await` between the removal and the re-insertion is a removal — cleanups run, `setup` runs again, state starts over. It matters for virtualised lists and animated re-parenting.
- README documents `cx` / `sx`, and what `{ dev: true }` actually turns on — the error message in the page, plus a table of the dev-only warnings and what each one catches.
...
- The a11y suite grew from 8 assertions to 20: contrast is now checked for every text token against every surface the library paints (a 5×3 matrix per theme) rather than two backgrounds, plus each role's soft badge/alert pairing, that muted stays visibly distinct from secondary, that pointer targets meet 24px, and that no rule anywhere strips an outline without replacing it.
- `test/jsdom/define-errors.test.mjs` — 11 tests over every rejected `define()` call and the message it produces, including that a failed re-registration leaves the first definition working. Nine fail against the previous implementation.
- `test/jsdom/rich-props.test.mjs` — 10 tests for the object/callback channel: the value is present before the child's `setup` runs, a callback can be invoked by the child and always runs the newest closure, replacing an object re-renders the child while handing back the same one does not, a fresh callback closure does not, `props` stays a string map, and string bindings are unaffected. Also pins the limitation that a tag name cannot be interpolated.
...
### Changed
- `test.html` emits a machine-readable `#summary[data-passed][data-failed][data-done]` alongside the text it already showed, which is what the headless runner reads.
- `define()` explains what is wrong instead of forwarding a `DOMException`. A name without a dash, an uppercase name, an empty or non-string name each said only "Name argument is not a valid custom element name"; registering a name twice said only "This name has already been registered in the registry", with no hint that a module running twice is the usual cause. Each case now names the tag, says what the rule is, and suggests the fix.
- A `setup` argument that is not a function is now rejected by `define()` itself. It used to be accepted silently and fail per element, much later, as `render is not a function` — with the stack pointing at the library. A `setup` that returns something other than a function (returning `` html`...` `` instead of `` () => html`...` ``, the usual slip) is reported the same way, through the component's own error path.
- README's size claim updated from ~5 KB to ~6.5 KB gzipped, which is what this release actually measures.
- The dark-theme parity check covers every token instead of six. The 40 dark values are necessarily written out twice — CSS cannot combine "the system is dark" (a media query) with "the theme attribute says dark" (a selector) into one rule — and a hand-picked list of six was the only thing keeping the two copies honest. The check now compares both dark blocks token for token, and additionally requires `[data-theme="light"]` to restore every token the dark blocks override, so an explicit light theme inside a dark page cannot end up half-themed.
...
### Fixed
- A binding that changes from a string to an object no longer leaves the old attribute behind. `setProp` writes a non-primitive as a DOM property and never touched the attribute, so `data=${"plain"}` followed by `data=${{n:9}}` left `data="plain"` in the DOM — and on a child component that stale attribute won, because `props` is rebuilt from the element's attributes on every render. The attribute is now removed when the value stops being one.
- Mounting a component whose render returns a hand-built vnode no longer throws. `connectedCallback` appended `tree.dom` directly, relying on the library having built the DOM eagerly; a vnode constructed by the caller — which `resolveBinding` explicitly accepts as a value — has no `dom` yet, so the append failed and the component showed the error box. The tree is materialized first.
- The last contrast failure is gone. `--ui-text-muted` cleared AA on `--ui-surface` and `--ui-background` but sat at **4.34:1 light / 4.42:1 dark** on `--ui-surface-muted` — reachable through `.ui-empty-icon`, which sets both, and any `.ui-muted` or `.ui-caption` inside a `.ui-table th` or a muted panel. It moves to `#5e6b80` (4.93:1 at worst) and `#94949e` (4.96:1), staying clearly lighter than `--ui-text-secondary`.
- Checkbox and radio click targets meet WCAG 2.2 SC 2.5.8. The native boxes were `1rem` (16px) against a 24px minimum. The label wrapping them is the real target, so `.ui-checkbox` and `.ui-radio` now carry `min-height: 1.5rem`, and the boxes themselves go to `1.125rem`.
- `.ui-input` no longer strips the browser's focus ring in its base rule. `outline: none` there removed the native ring in every state, not just the one the `:focus` rule replaces; the focus rule sets its own transparent outline, so the blanket removal had nothing left to do.
- An explicit `data-theme="light"` no longer depends on source order to beat the system's dark preference. The dark media query selected a bare `:root`, which has the same specificity as `[data-theme="light"]`, so light-inside-dark-mode worked only because that block happens to sit lower in the file. The media query now selects `:root:not([data-theme="light"])` and wins or loses on specificity, as intended.
- A child component with an object or callback binding no longer re-renders every time its parent does. The props-sync loop stringified a non-string binding to `"[object Object]"` and wrote it into `props`, while `syncProps` deletes any prop that is not a real attribute on the child's own next render — so the value was gone again by the following pass and always looked like a change. Measured: five parent updates with unchanged rows caused **100 child renders across 20 children; now 0**. Object and array bindings are compared by identity, and a callback — read when it is called, not when the child renders — updates the property without forcing a render.
...
### Documentation
- README API reference gains `onReady` and `onError`. Both are exported and both appear in the usage examples, but neither had an entry beside every other export — including the cleanup contract, that a move is not a removal, that each `onReady` callback is isolated, and that a component is its own error boundary.
- README documents the two restrictions that fail in ways that do not look like the caller's mistake: an `on*` attribute must be an interpolated function rather than a string of code (which throws, `html.raw` included), and a tag name cannot be interpolated (which renders as escaped text rather than raising, since each template is parsed once).
- README's "What This Is Not" no longer claims the library is "not optimized (v0 uses a full internal tree for correctness)". Keyed placement is now O(n log n) with a minimum-move plan, an unchanged list costs no DOM writes, and handlers do not churn the listener list — the honest limits are the missing framework pieces and the absence of SSR, which is what it says instead.
- README's size claim corrected from ~6.5 KB to ~6.3 KB gzipped, and the Development section now lists `tsr test:browser`, the optional browser prerequisite, and the current test count.
- README documents how to pass objects, arrays and callbacks to a child component, which was not written down anywhere: non-string bindings become DOM properties rather than `props` entries, objects are compared by identity, callbacks are read when called, and an `on*` name is an event binding rather than a prop.
...
### Removed
- `removeListener` from `dom.ts`. Nothing has called it since handlers moved to slots — `patchEvents` empties a slot instead of detaching a listener — and it was never part of the public API.
...
- FakeDOM test harness — removed `test/helpers/dom.mjs` and all `test/micro-ui.*.test.mjs` (FakeDOM) suites. All coverage is now provided by the jsdom suite under `test/jsdom/` (`node --test`). `tasks.toml` updated: `[tasks.test]` now runs the package's Node test script and `[tasks.test:e2e]` (esdev FakeDOM) is removed.
...
- jsdom tests run under Node; FakeDOM/E2E tests continue under **esdev**.
