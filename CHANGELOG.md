...
- README no longer says a cache-busting query gives each test file its own module instance. It does not: under the bun in use every `src/index.ts?whatever` resolves to the same module, so `devMode` and everything else at module scope is shared by the whole run — which is exactly how one test came to depend on the order files happened to run in. The Development section now says to set `{ dev: true }` or `{ dev: false }` explicitly rather than assuming the default.
