...
test("attrs: an external change alone does not re-render", async () => {
  ...
  assertEquals(c.el.textContent, "1");
  ...
  assertEquals(c.renders(), before, "nothing re-renders on its own");
  assertEquals(c.el.textContent, "1");
});
...
test("attrs: an explicit update() picks up an external change", async () => {
  ...
  update(c.el);
  await tick();
  assertEquals(c.el.textContent, "2", "update() must render current truth");
});
...
test("attrs: removing an attribute deletes the prop", async () => {
  ...
  update(c.el);
  await tick();
  assertEquals("n" in c.props(), false, "a removed attribute is an absent prop");
  assertEquals(c.el.textContent, "undefined");
});

test("attrs: an attribute absent at connect is picked up later", async () => {
  const c = await mount({ n: "1" });
  assertEquals("label" in c.props(), false);
  c.el.setAttribute("label", "hello");
  update(c.el);
  await tick();
  assertEquals(c.props().label, "hello");
});
...
test("attrs: the props object identity is preserved", async () => {
  ...
  update(c.el);
  await tick();
  assert(c.props() === original, "props must be mutated in place");
  assertEquals(original.n, "2");
});
...
test("attrs: repeated updates with no attribute change render once each", async () => {
  ...
  update(c.el);
  await tick();
  assertEquals(c.renders(), before + 2, "no extra renders from the re-read");
});
...
test("attrs: values are read as strings, matching connect-time behaviour", async () => {
  ...
  update(c.el);
  await tick();
  assertEquals(c.props().n, "42");
  assertEquals(typeof c.props().n, "string");
});
...
test("attrs: parent-driven attribute changes still propagate", async () => {
  ...
  document.body.appendChild(el);
  await tick();
  assertEquals(el.querySelector("x-attr-child").textContent, "one");
  ...
  await tick();
  await tick();
  assertEquals(el.querySelector("x-attr-child").textContent, "two");
  assertEquals(renders, 2, "exactly one extra render, not two");
});
...
test("attrs: a nullish parent binding still does not re-render the child", async () => {
  ...
  assertEquals(renders, 1, "child attrs unchanged, so no re-render");
});