...
const { define, html, update, flush, store } = await import("../../src/index.ts");
...
test("jsdom setProp: value null clears input.value", async () => {
  ...
  assertEquals(inp.value, "hi");
  el._setNull();
  await tick(); flush();
  assertEquals(inp.value, "");
  assertEquals(inp.getAttribute("value"), null);
});
...
test("jsdom setProp: value string sets both .value and attribute", async () => {
  ...
  await tick(); flush();
  const inp = el.querySelector("input");
  assertEquals(inp.value, "test");
  assertEquals(inp.getAttribute("value"), "test");
});
...
test("jsdom setProp: value number coerces to string", async () => {
  ...
  await tick(); flush();
  const inp = el.querySelector("input");
  assertEquals(inp.value, "42");
  assertEquals(inp.getAttribute("value"), "42");
});
...
test("jsdom setProp: disabled false removes attribute on real DOM", async () => {
  ...
  el._toggle();
  await tick(); flush();
  assertEquals(btn.hasAttribute("disabled"), false);
});
...
test("jsdom setProp: checked toggle on real checkbox", async () => {
  ...
  await tick(); flush();
  const inp = el.querySelector("input");
  assertEquals(inp.checked, true);
  el._toggle();
  await tick(); flush();
  assertEquals(inp.checked, false);
});
...
test("jsdom setProp: selected boolean reflects .selected and attribute", async () => {
  ...
  assertEquals(opt.selected, true);
  assertEquals(opt.hasAttribute("selected"), true);
});
...
test("jsdom setProp: indeterminate boolean sets .indeterminate", async () => {
  ...
  document.body.appendChild(el);
  await tick(); flush();
  assertEquals(el.querySelector("input").indeterminate, true);
});
...
test("jsdom setProp: aria-hidden null removes attribute on update", async () => {
  ...
  update(ref);
  await tick(); flush();
  assertEquals(el.querySelector("div").getAttribute("aria-hidden"), null);
});
...
test("jsdom setProp: null removes class attribute", async () => {
  ...
  document.body.appendChild(el);
  await tick(); flush();
  assertEquals(el.querySelector("div").getAttribute("class"), "active");
  el._clear();
  await tick(); flush();
  assertEquals(el.querySelector("div").getAttribute("class"), null);
});
...
test("jsdom setProp: true sets hidden via the BOOLEAN_PROPS path", async () => {
  ...
  assertEquals(div.hasAttribute("hidden"), true);
  assertEquals(div.hidden, true, "boolean prop must reflect to the property");
});
...
test("jsdom setProp: addListener fires after keyed reorder", async () => {
  ...
  btns[0].click();
  await tick(); flush();
  assertEquals(btns[0].textContent, "1");
...
test("jsdom keyed list: add + remove with hoisted getParentNS", async () => {
  ...
  await tick(); flush();
  assertEquals(el.querySelectorAll("li").length, 2);
  assertEquals([...el.querySelectorAll("li")].map((n) => n.textContent).join(","), "b,c");
});
...
test("jsdom store: deeply nested path set/get/delete", async () => {
  store.clear();
  store.set("d", { a: { b: { c: 1 } } });
  assertEquals(store.get("d", { path: "a.b.c" }), 1);
  store.set("d", 99, { path: "a.b.c" });
  assertEquals(store.get("d", { path: "a.b.c" }), 99);
  store.del("d", { path: "a.b.c" });
  assertEquals(store.get("d", { path: "a.b.c" }), undefined);
});
...
test("jsdom setProp: value property syncs .value and attribute (port)", async () => {
  ...
  val = "world"; update(ref); await tick(); flush();
...
test("jsdom setProp: onclick handler fires after reconciliation (port)", async () => {
  ...
  const el = document.createElement(tag); document.body.appendChild(el); await tick(); flush();
  el.querySelector("button").click(); await tick(); flush();
...
test("jsdom setProp: non-primitive value is written as a property, not an attribute", async () => {
  ...
  await tick(); flush();
  const div = el.querySelector("div");
  assert(div["data-obj"] === payload, "setElProp writes the raw value");
  assertEquals(div.getAttribute("data-obj"), null, "objects do not stringify to an attribute");
});