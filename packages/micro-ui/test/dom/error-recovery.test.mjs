...
const { define, html, update, flush, onError, onReady } = await import("../../src/index.ts");
...
test("recovery: fixing the cause and calling update brings the component back", async () => {
  ...
  await tick();
  assertEquals(el.textContent, "healthy");

  broken = true;
  update(ref);
  await tick();
  flush();
  assert(el.querySelector("[data-micro-ui-error]"), "it failed");
  ...
  assertEquals(el.querySelector("[data-micro-ui-error]"), null, "box is gone");
  assertEquals(el.textContent, "healthy", "and the component rendered again");
});
...
test("recovery: the recovered component keeps re-rendering normally", async () => {
  ...
  await tick();
  flush();
  assertEquals(el.textContent, "7");

  n = 8;
  update(ref);
  await tick();
  flush();
  assertEquals(el.textContent, "8", "not a one-shot recovery");
});
...
test("recovery: no leftovers from the error box or the pre-failure tree", async () => {
  ...
  assertEquals(el.querySelectorAll("[data-micro-ui-error]").length, 0);
  assertEquals(el.querySelectorAll("ul").length, 1, "one list, not two");
  assertEquals(el.querySelectorAll("li").length, 2);
  assertEquals(el.textContent, "onetwo");
});
...
test("recovery: a failure in reconcile recovers too", async () => {
  ...
  if (failed) {
    assertEquals(el.querySelector("[data-micro-ui-error]"), null);
  }
...
test("recovery: onError still fires on every failure, not just the first", async () => {
  ...
  assertEquals(phases, ["render", "render"], "reported both times");
  assertEquals(el.textContent, "ok");
});
...
test("recovery: cleanups registered before the failure still run on removal", async () => {
  ...
  assertEquals(cleaned, 1, "the error round trip did not lose the cleanup");
});