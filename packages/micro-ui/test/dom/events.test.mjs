...
test("events: a re-render swaps in the new closure", async () => {
  ...
  assertEquals(calls, [0, 1, 2], "each click ran the newest closure");
});
...
test("events: the handler closes over the row it was rendered with", async () => {
  ...
  await tick();
  click(el.querySelectorAll("button")[1]);
  assertEquals(clicked, ["B", "B2"], "not the stale closure");
});
...
test("events: handlers follow their row through a reorder", async () => {
  ...
  assertEquals(buttons.map((b) => b.textContent), ["c", "a", "b"]);
  click(buttons[0]);
  assertEquals(clicked, ["c"], "the moved row kept its own handler");
});
...
test("events: dropping the binding stops the handler firing", async () => {
  ...
  assertEquals(hits, 1, "the emptied slot must not fire");

  armed = true;
  rerender();
  await tick();
  click(el.querySelector("button"));
  assertEquals(hits, 2, "and re-binding works after the slot was emptied");
});
...
test("events: the handler receives the event and `this` is the element", async () => {
  ...
  const btn = el.querySelector("button");
  click(btn);
  assert(seenTarget === btn, "event is passed through");
  assert(seenThis === btn, "`this` is the element, as with addEventListener");
});
...
test("events: re-rendering does not touch the listener list", async () => {
  ...
  assertEquals(el.querySelectorAll("button").length, 25);
  ...
  assertEquals(adds, 0, "no listener was added across three re-renders");
  assertEquals(removes, 0, "and none was removed");
  assertEquals(el.querySelector("button").textContent, "3", "but it did render");
});
...
test("events: a brand new row binds exactly one listener", async () => {
  ...
  assertEquals(el.querySelectorAll("button").length, 2);
  assertEquals(adds, 1, "one listener for the one new button");
});