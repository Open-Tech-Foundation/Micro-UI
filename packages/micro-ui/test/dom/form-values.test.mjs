...
test("a select whose options are interpolated", async () => {
  ...
  assertEquals(el.querySelector("select").value, "linus");
});
...
test("a select keeps its binding when the selected option is removed", async () => {
  ...
  const sel = el.querySelector("select");
  assertEquals(sel.value, "linus");

  // Grace leaves; the binding still says linus, and linus is still there.
  state.people = ["ada", "linus"];
  await rerender();
  assertEquals(sel.value, "linus", "removing another option stole the selection");
});
...
test("a textarea with a value binding", async () => {
  ...
  assertEquals(ta.value, "one");
  state.text = "two";
  await rerender();
  assertEquals(ta.value, "two");
});
...
test("the caret survives a value the app rewrote", async () => {
  ...
  assertEquals(input.value, "HELXLO");
  assertEquals(input.selectionStart, 4, "the caret jumped");
  assertEquals(input.selectionEnd, 4);
});
...
test("a shorter value clamps the caret instead of throwing", async () => {
  ...
  assertEquals(input.selectionStart, 2);
});
...
test("a selection range is kept, not collapsed", async () => {
  ...
  state.text = "hello!";
  await rerender();
  assertEquals(input.selectionStart, 1);
  assertEquals(input.selectionEnd, 4);
});
...
test("an unfocused input is left alone", async () => {
  ...
  assert(document.activeElement === other, "focus moved");
});
...
test("a value the DOM already holds is not written again", async () => {
  ...
  state.text = "ab";
  await rerender();
  assertEquals(input.selectionStart, 1, "an identical value still moved the caret");
});
...
test("a select recovers its binding when the selection is destroyed", async () => {
  ...
  await rerender();
  assertEquals(sel.value, "grace", "the binding was not restored");
});
...
test("a select follows its binding when the options shift under it", async () => {
  ...
  await rerender();
  assertEquals(sel.value, "grace", "the selection slid onto another option");
});