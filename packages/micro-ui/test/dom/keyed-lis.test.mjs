...
function sameNodes(actual, expected, msg) {
  assertEquals(actual.length, expected.length, `${msg}: row count`);
  for (let i = 0; i < expected.length; i++)
    assert(actual[i] === expected[i], `${msg}: row ${i} is not the same node`);
}
...
test("keyed: swapping two rows of 1001 costs two moves, not the span", async () => {
  ...
  assertEquals(ul.children.length, 1001);
  ...
  assert(ul.children[1] === second, "row 998 moved up");
  assert(ul.children[998] === first, "row 1 moved down");
  assertEquals(ul.children[1].textContent, "998");
  assertEquals(ul.children[998].textContent, "1");
  assertEquals(ul.children.length, 1001);
  assertEquals(moves.calls, 2, "one insertBefore per genuinely displaced row");
});
...
test("keyed: re-rendering an unchanged list moves nothing", async () => {
  ...
  await tick();
  moves.restore();
  assertEquals(moves.calls, 0);
  assertEquals(ul.children.length, 50);
});
...
test("keyed: changing only row content moves nothing", async () => {
  ...
  assertEquals(text(ul), "b1,b2,b3,b4,b5");
});
...
test("keyed: moving one row to the front is a single move", async () => {
  ...
  assertEquals(text(ul), "6,1,2,3,4,5");
  assert(ul.children[0] === sixth, "identity preserved");
  assertEquals(moves.calls, 1);
});
...
test("keyed: reversing n rows costs n-1 moves", async () => {
  ...
  assertEquals(text(ul), "8,7,6,5,4,3,2,1");
  assertEquals(moves.calls, 7, "one row of the reversal can stay put");
});
...
test("keyed: appending rows does not move the rows already there", async () => {
  ...
  assertEquals(text(ul), "1,2,3,4,5");
  sameNodes([...ul.children].slice(0, 3), before, "existing rows untouched");
  assertEquals(moves.calls, 2, "only the two new rows are inserted");
});
...
test("keyed: shuffles converge, with no more moves than rows out of order", async () => {
  ...
  for (let round = 0; round < 25; round++) {
    ...
    assertEquals(text(ul), next.join(","), `round ${round} order`);
    assertEquals(moves.calls, optimal, `round ${round} move count`);
    for (const n of ul.children)
      assert(n === nodes.get(n.textContent), `round ${round} identity`);
  }
});
...
test("keyed: add, remove and reorder in one pass", async () => {
  ...
  rerender();
  await tick();
  assertEquals(text(ul), "5,9,3,1,7");
  assertEquals(ul.children.length, 5);
  assert(ul.children[2] === three, "surviving row kept its node");
});
...
test("keyed: unkeyed rows mixed into a keyed list stay positional", async () => {
  ...
  const { ul, rerender } = await mountList(items, row);
  assertEquals(text(ul), "1,2,3");
  items.current = [3, 2, 1];
  rerender();
  await tick();
  assertEquals(text(ul), "3,2,1");
...
test("keyed: a duplicated key leaves exactly one row behind", async () => {
  ...
  rerender();
  await tick();
  assertEquals(ul.children.length, 3, "no orphan from the shadowed duplicate");
  assertEquals(text(ul), "1,1,3");
});
...
test("keyed: a row detached from outside is put back, not rebuilt", async () => {
  ...
  assertEquals(ul.children.length, 3);
  assert(ul.children[1] === second, "same node reclaimed by key");
  assertEquals(text(ul), "1,2,3");
});
...
test("keyed: a list re-ordered from outside converges on the next render", async () => {
  ...
  assertEquals(text(ul), "4,1,2,3");

  rerender();
  await tick();

  assertEquals(text(ul), "1,2,3,4", "the render's order wins");
});
...
test("keyed: rows changing tag are replaced, not duplicated", async () => {
  ...
  assertEquals(ul2.children.length, 3);
  assertEquals(ul2.children[1].tagName, "B");
  rerender2();
  await tick();
  assertEquals(ul2.children.length, 3, "no resurrected node");
  assertEquals(text(ul2), "1,2,3");
  rerender();
  await tick();
  assertEquals(ul.children.length, 3);
});
...
test("keyed: emptying and refilling a large list", async () => {
  ...
  rerender();
  await tick();
  assertEquals(ul.children.length, 0);
  items.current = Array.from({ length: 200 }, (_, i) => i + 1000);
  rerender();
  await tick();
  assertEquals(ul.children.length, 200);
  assertEquals(ul.children[0].textContent, "1000");
  assertEquals(ul.children[199].textContent, "1199");
});
...
test("keyed: fast path replaces a row whose tag changed, in place", async () => {
  ...
  assertEquals(ul.children[1].tagName, "LI");

  heavy = true;
  rerender();
  await tick();

  assertEquals(ul.children.length, 3, "replaced, not duplicated");
  assertEquals(ul.children[1].tagName, "B");
  assertEquals(text(ul), "1,2,3");
  assert(ul.children[0] === first, "its neighbours were left alone");
});
...
test("keyed: numeric and string keys collide without leaking rows", async () => {
  ...
  const assertRows = (label) => {
    ...
    assertEquals(ul.children.length, items.current.length, `${label}: count`);
  };
