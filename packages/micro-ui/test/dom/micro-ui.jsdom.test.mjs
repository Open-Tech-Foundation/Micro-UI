...
test("jsdom: counter renders initial value from attribute", async () => {
  ...
  assertEquals(btn.textContent.trim(), "Count: 7");
});
...
test("jsdom: onclick handler mutates state and update re-renders in place", async () => {
  ...
  assertEquals(btn.textContent.trim(), "n=0");
  assertEquals(span.textContent, "even");

  btn.click();
  await tick();
  flush();

  assertEquals(btn.textContent.trim(), "n=1");
  assertEquals(span.textContent, "odd");

  // DOM identity: the same button element survived the re-render.
  assert(el.querySelector("button.inc") === btnNode);
});
...
test("jsdom: keyed list reorder keeps stable DOM identity", async () => {
  ...
  assertEquals(appleNode.getAttribute("data-id"), "a");
  ...
  assertEquals(after[0].getAttribute("data-id"), "c");
  assertEquals(after[1].getAttribute("data-id"), "b");
  assertEquals(after[2].getAttribute("data-id"), "a");
  // Identity preserved by key.
  assert(after[0] === cherryNode);
  assert(after[1] === bananaNode);
  assert(after[2] === appleNode);
});
...
test("jsdom: form input two-way binding keeps focus on the same element across renders", async () => {
  ...
  assert(el.querySelector("input") === inputNode, "input DOM node must be reused");
  assertEquals(input.value, "ada");
  assertEquals(mirror.textContent, "ada");
});
...
test("jsdom: interpolated text is inserted as literal text, never markup", async () => {
  ...
  assertEquals(p.querySelector("script"), null, "script tags must not be created");
  assertEquals(p.querySelector("b"), null, "injected markup must not be parsed");
  assertEquals(globalThis.window.__pwned, undefined, "script must not have executed");
  ...
  assertEquals(tn.nodeType, 3);
  assertEquals(tn.data, evilText);
  assertEquals(p.textContent, evilText, "user sees the literal source text");
});
...
test("jsdom: ampersand, quotes and angle brackets round-trip verbatim", async () => {
  ...
  assertEquals(tn.data, `a & b <c> "d" 'e'`);
  assertEquals(span.textContent, `a & b <c> "d" 'e'`);
});
...
test("jsdom: html.raw injects trusted markup unescaped", async () => {
  ...
  assert(b, "raw markup should be parsed into real elements");
...
test("jsdom: nested html`` text bindings stay literal text", async () => {
  ...
  assertEquals(span.querySelector("b"), null);
  const tn = span.firstChild;
  assertEquals(tn.nodeType, 3);
  assertEquals(tn.data, "<b>nope</b>");
  assertEquals(span.textContent, "<b>nope</b>");
});
...
test("jsdom: initial render that throws mounts an error fallback", async () => {
  ...
  assert(box, "error UI should be mounted");
  assert(box.textContent.includes("kaboom-init"));
});
...
test("jsdom: static on* string attribute is rejected with actionable guidance", async () => {
  ...
  flush();
  const box = el.querySelector("[data-micro-ui-error]");
  assert(box, "static on* string should trigger the error fallback");
  assert(
  ...
  assert(!box.textContent.includes("addEventListener"), "should not surface a DOM TypeError");
});
...
test("jsdom: update that throws does not poison sibling re-renders", async () => {
  ...
  await tick();
  flush();
  assertEquals(bad.querySelector("p").textContent, "ok");
  assertEquals(good.querySelector("p").textContent, "count=0");
  ...
  await tick();
  flush();
  assertEquals(bad2.querySelector("p").textContent, "fine");
  bad2.dispatchEvent(new Event("force", { bubbles: true }));
  await tick();
  flush();
  assert(bad2.querySelector("[data-micro-ui-error]"), "bad2 should show error UI");
  assert(bad2.textContent.includes("bad-update-2"));
});
...
test("jsdom: onError handler receives the failing element, error, and phase", async () => {
  ...
  assertEquals(seen[0].message, "render-bang");
  assert(seen[0].tag.length > 0);
});
...
test("jsdom: onError handler that throws is logged but does not break the host", async () => {
  ...
  try {
    ...
    assert(found, "handler throw should be reported via console.error");
  } finally {
...
test("jsdom: a component that keeps throwing keeps showing the error box", async () => {
  ...
  assert(again, "still showing an error box");
  assertEquals(el.querySelectorAll("[data-micro-ui-error]").length, 1, "exactly one");
  assertEquals(el.querySelector("p"), null, "and no half-rendered content");
});
...
test("jsdom: keyed node moved to another parent is not stolen or crash on reorder", async () => {
  ...
  assertEquals(el.querySelector("[data-micro-ui-error]"), null, "must not crash");
  const labels = [...el.querySelectorAll("li")].map((n) => n.textContent);
  assertEquals(labels, ["Gamma", "Alpha", "Beta"], "list is correctly reordered");
  // The moved node stays put in its new parent; the ul gets a fresh node.
  assert(liB.parentNode === elsewhere, "moved node is not stolen back");
  assertEquals(elsewhere.childElementCount, 1);
});
...
test("jsdom: unkeyed list update reuses DOM nodes by position", async () => {
  ...
  assert(after[0] === span0);
  assert(after[1] === span1);
});
...
test("jsdom: new keyed items get fresh DOM on insert", async () => {
  ...
  assertEquals(liA.textContent, "a");
  ...
  assertEquals(all[0].textContent, "z");
  assertEquals(all[1].textContent, "a");
  // Original 'a' node preserved
  assert(all[1] === liA);
  // New 'z' node is a real DOM element
  assertEquals(all[0].nodeType, 1);
});
...
test("jsdom: event listeners survive keyed reorder without re-attachment churn", async () => {
  ...
  await tick();
  flush();
  assertEquals(btnA.textContent, "1");
  ...
  assert(afterBtns[0] === btnB);
  assert(afterBtns[1] === btnA);

  // Click button A again (now at index 1)
  afterBtns[1].click();
  await tick();
  flush();
  assertEquals(afterBtns[1].textContent, "2");
});
...
test("jsdom: conditional show/hide creates and discards DOM correctly", async () => {
  ...
  const span = el.querySelector("span");
  assert(span);
  assertEquals(span.textContent, "extra");
  ...
  assertEquals(el.querySelector("span"), null);
  // Persistent element reused
  assert(el.querySelector("p") === p);
  ...
  assert(span2);
  assertEquals(span2.textContent, "extra");
...
test("jsdom: a self-update during render is bounded, never an unbounded cascade", async () => {
  ...
  assert(renders > 1, "the deferred self-update runs at least once");
  assert(renders < 60, `the chain must terminate, got ${renders} renders`);
  assert(/render loop/.test(reported ?? ""), "and the loop is reported");
  assert(el.querySelector("[data-micro-ui-error]"));
});
test("jsdom: aria attributes stringify booleans correctly", async () => {
  ...
  await tick(); flush();
  div = el.querySelector("div");
  assertEquals(div.hasAttribute("aria-hidden"), false);
});
...
test("jsdom: aria-expanded toggle keeps correct string values", async () => {
  ...
  assertEquals(el.querySelector("#panel").getAttribute("aria-hidden"), "true");
  el._toggle();
  await tick(); flush();
  assertEquals(el.querySelector("button").getAttribute("aria-expanded"), "true");
  assertEquals(el.querySelector("#panel").getAttribute("aria-hidden"), "false");
});
...
test("jsdom: aria boolean true/false via direct boolean binding", async () => {
  ...
  assertEquals(div.getAttribute("aria-valuenow"), "42");
});
...
test("html: number and boolean rendering", async () => {
  ...
  const el = await mountEl(tag);
  assert(el.textContent.includes("0"));
  assert(el.textContent.includes("yes"));
});
...
test("html: nested fragment", async () => {
  ...
  assert(el.querySelector("b") !== null);
});
...
test("attributes: removal when null/false", async () => {
  ...
  update(ref);
  await settle();
  assertEquals(el.querySelector("div").getAttribute("title"), null);
  title = false;
  update(ref);
  await settle();
  assertEquals(el.querySelector("div").getAttribute("title"), null);
...
test("attributes: boolean disabled/selected sync", async () => {
  ...
  update(ref);
  await settle();
  assertEquals(el.querySelector("button").getAttribute("disabled"), null);
});
...
test("props: checked boolean toggle", async () => {
  ...
  const el = await mountEl(tag);
  assertEquals(el.querySelector("input").checked, true);
  assert(el.querySelector("input").getAttribute("checked") !== null);
  v = false;
  update(ref);
  await settle();
  assertEquals(el.querySelector("input").checked, false);
  assertEquals(el.querySelector("input").getAttribute("checked"), null);
});
...
test("events: handler replacement (old removed)", async () => {
  ...
  const el = await mountEl(tag);
  el.querySelector("button").click();
  assertEquals(h1calls, 1);
  h = () => h2calls++;
  update(ref);
  await settle();
  el.querySelector("button").click();
  assertEquals(h1calls, 1);
  assertEquals(h2calls, 1);
});
...
test("lists: keyed reorder preserves DOM order", async () => {
  ...
  update(ref);
  await settle();
  assertEquals([...el.querySelectorAll("li")].map(n => n.textContent).join(","), "3,1,2");
});
...
test("lists: detached keyed node is reclaimed by key, not recreated", async () => {
  ...
  update(ref);
  await settle();
  assertEquals(ul.querySelectorAll("li").length, 2);
  assert(ul.querySelectorAll("li")[0] === one, "detached keyed node reclaimed by key (identity preserved)");
});
...
test("define: multiple instances isolated (closure + props)", async () => {
  ...
  await tick();
  assertEquals(a.querySelector("span").textContent, "0");
  assertEquals(b.querySelector("span").textContent, "10");
  a.querySelector("button").click();
  await settle();
  assertEquals(a.querySelector("span").textContent, "1");
  assertEquals(b.querySelector("span").textContent, "10");
});
...
test("onReady: throws when called outside define", async () => {
  ...
  try {
    ...
    threw = true;
    assert(e.message.includes("onReady"));
  }
...
test("mount: clears host and appends child", async () => {
  ...
  await delay();
  assert(host.textContent.includes("child"));
...
test("reconcile: tag mismatch replaces element", async () => {
  ...
  await settle();
  assert(el.querySelector("span") !== null);
  assertEquals(el.querySelector("div"), null);
});
...
test("reconcile: attribute patch removes old attr", async () => {
  ...
  const el = await mountEl(tag);
  assertEquals(el.querySelector("div").getAttribute("title"), "x");
  withTitle = false;
  update(ref);
  await settle();
  assertEquals(el.querySelector("div").getAttribute("title"), null);
...
test("html.raw: does not rebuild DOM when structure is unchanged across updates", async () => {
  ...
  try {
    ...
    assertEquals(created, 0, `raw must reuse its DOM on a same-structure update (created ${created})`);
...
test("onError: handler receives (el, error, phase)", async () => {
  ...
  assertEquals(captured.err.message, "boom");
});
...
test("props: disabled 'false' string means not disabled", async () => {
  ...
  const el = await mountEl(tag);
  assertEquals(el.querySelector("button").getAttribute("disabled"), null);
  assertEquals(el.querySelector("button").disabled, false);
});
test("props: disabled '0' / 'off' strings mean not disabled", async () => {
  ...
  assertEquals(a.getAttribute("disabled"), null);
  assertEquals(b.getAttribute("disabled"), null);
});
test("props: selected 'false' string means not selected", async () => {
  ...
  const el = await mountEl(tag);
  assertEquals(el.querySelector("option").getAttribute("selected"), null);
  ...
  assertEquals(el.querySelector("option").hasAttribute("selected"), false);
});
test("props: readonly 'false' string means not readonly", async () => {
  ...
  const el = await mountEl(tag);
  assertEquals(el.querySelector("input").getAttribute("readonly"), null);
});
test("props: hidden 'false' string means not hidden", async () => {
  ...
  const el = await mountEl(tag);
  assertEquals(el.querySelector("div").getAttribute("hidden"), null);
});
test("props: boolean toggles stay consistent (control)", async () => {
  ...
  const el = await mountEl(tag);
  assert(el.querySelector("button").getAttribute("disabled") !== null);
  assertEquals(el.querySelector("button").getAttribute("required"), null);
  dis = false;
  req = true;
  update(ref);
  await settle();
...
test("order: event handler written before attribute binding", async () => {
  ...
  btn.click();
  assertEquals(clicked, true, "click must invoke the bound handler");
});
...
test("order: keyed list with id after key (control, key first)", async () => {
  ...
  update(ref);
  await settle();
  assertEquals([...el.querySelectorAll("li")].map(n => n.getAttribute("id")).join(","), "2,1");
});
...
test("aria: null/undefined removes aria attribute", async () => {
  ...
  update(ref);
  await settle();
  assertEquals(el.querySelector("div").getAttribute("aria-label"), null);
  label = undefined;
  update(ref);
  await settle();
  assertEquals(el.querySelector("div").getAttribute("aria-label"), null);
