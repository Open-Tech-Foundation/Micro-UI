...
const { define, html, update, onReady, onError } = await import("../../src/index.ts");
...
test("move: re-parenting does not re-run setup or cleanups", async () => {
  ...
  assertEquals(setups, 1, "setup must not re-run on a move");
  assertEquals(cleanups, 0, "cleanups must not fire on a move");
  assert(el.parentNode === b);
});
...
test("move: component state survives re-parenting", async () => {
  ...
  update(ref);
  await tick();
  assertEquals(el.textContent, "3", "counter kept counting, it did not reset");
});
...
test("move: a real removal still tears the component down", async () => {
  ...
  assertEquals(cleanups, 1, "a genuine disconnect must still clean up");

  document.body.appendChild(el);
  await tick();
  assertEquals(setups, 2, "re-adding later is a fresh mount");
});
...
test("reentry: a self-update during render is applied, not dropped", async () => {
  ...
  assertEquals(renders, 3, "the deferred self-update must run, not be dropped");
...
test("reentry: a runaway self-update reports instead of hanging", async () => {
  ...
  assert(caught, "the loop must surface through onError");
  assertEquals(caught.phase, "render");
  expect(caught.message).toMatch(/render loop/);
  assert(el.querySelector("[data-micro-ui-error]"), "and mount the error UI");
});
...
test("reentry: a bounded self-update does not trip the loop guard", async () => {
  ...
  assertEquals(errored, false, "a couple of passes is legitimate, not a loop");
  assertEquals(renders, 4);
});
...
test("reentry: update() during the mount render is a no-op", async () => {
  ...
  assertEquals(renders, 1, "no loop, and no deferred re-render");
  assertEquals(el.textContent, "1");
});
...
test("reentry: repeated short self-update bursts never trip a false loop", async () => {
  ...
  assertEquals(errored, null, `bursts must not be treated as a loop: ${errored}`);
  assertEquals(remaining, 0);
  assert(renders > 40, `all bursts ran, got ${renders} renders`);
});