...
import { test, assert, assertEquals, expect } from "runtime:test";
...
test("regression: '&' in interpolated text is not rendered as '&amp;'", async () => {
  ...
  assertEquals(el.querySelector("p").textContent, "Tom & Jerry");
  assertEquals(el.innerHTML, "<p>Tom &amp; Jerry</p>");
});

test("regression: '<' in interpolated text is not rendered as '&lt;'", async () => {
  const el = await mount(() => () => html`<b>${"5 < 10"}</b>`);
  assertEquals(el.querySelector("b").textContent, "5 < 10");
});

test("regression: apostrophes and quotes survive interpolation", async () => {
  const el = await mount(() => () => html`<p>${`it's "fine"`}</p>`);
  assertEquals(el.querySelector("p").textContent, `it's "fine"`);
});

test("regression: text updates through reconcile stay unencoded", async () => {
  let v = "a & b";
  const el = await mount(() => () => html`<p>${v}</p>`);
  assertEquals(el.querySelector("p").textContent, "a & b");
  v = "c < d & e";
  update(el);
  await tick();
  assertEquals(el.querySelector("p").textContent, "c < d & e");
});
...
test("regression: interpolated markup is still inert (not parsed)", async () => {
  ...
  assertEquals(el.querySelector("img"), null, "must not create elements");
  assertEquals(globalThis.__pwned, undefined, "must not execute");
  assertEquals(el.querySelector("p").textContent, payload, "shown verbatim");
});
...
async function clearsCleanly(name, initial, render, sel, attr) {
  test(`regression: clearing ${name} removes it, not "undefined"`, async () => {
    ...
    assertEquals(node.getAttribute(attr), initial);
    ...
    assertEquals(node.hasAttribute(attr), false);
  });
}
...
test("regression: a cleared img[src] issues no request for '/undefined'", async () => {
  ...
  assertEquals(img.getAttribute("src"), null);
...
test("regression: patching a keyed list creates no throwaway elements", async () => {
  ...
  assertEquals(created, 0, "reusing 50 keyed rows must allocate no elements");
  assertEquals(el.querySelectorAll("li").length, 50);
  assertEquals(el.querySelectorAll("li")[7].textContent, "999");
});
...
test("regression: reordering a keyed list creates no throwaway elements", async () => {
  ...
  assertEquals(created, 0, "a pure reorder must move nodes, not rebuild them");
  assertEquals(
    [...el.querySelectorAll("li")].map((n) => n.textContent).join(""),
    "54321",
  );
  assert(el.querySelectorAll("li")[4] === first, "node identity is preserved");
});
...
test("regression: a keyed row that changes tag is replaced, not resurrected", async () => {
  ...
  assertEquals(el.querySelector("section").children.length, 2);
  ...
  assertEquals(kids.length, 2, "the replaced node must not linger alongside its replacement");
  assertEquals(kids[0].tagName, "DIV");
  assertEquals(kids[1].tagName, "I");
  assertEquals(el.querySelector("span"), null, "old node must be gone");
});
...
test("regression: nullish attr does not re-render a child on every parent update", async () => {
  ...
  assert(child, "child element must actually be in the tree");
  assertEquals(child.textContent, "hi", "child must actually have rendered");
  assertEquals(nestedChildRenders, 1);
  assertEquals(child.hasAttribute("data-x"), false, "nullish attr is removed");
  ...
  assertEquals(el.querySelector("b").textContent, "3", "parent did re-render");
  assertEquals(
    nestedChildRenders,
    1,
...
test("regression: a child still re-renders when its attributes do change", async () => {
  ...
  assertEquals(el.querySelector("x-reg-sync-child").textContent, "one");

  label = "two";
  update(el);
  await tick();
  await tick();
  assertEquals(renders, 2, "a real attribute change must still propagate");
  assertEquals(el.querySelector("x-reg-sync-child").textContent, "two");
});
...
test("regression: writing through an array path keeps it an array", () => {
  ...
  assert(Array.isArray(v.list), "list must still be an array");
  assertEquals(v.list, [9, 2, 3]);
});
...
test("regression: nested arrays survive a deep path write", () => {
  ...
  assert(Array.isArray(store.get("s3").a.b));
  assertEquals(store.get("s3").a.b[0].c, 2);
});
...
test("regression: deleting an array index splices instead of leaving a hole", () => {
  ...
  assertEquals(list.length, 2, "no sparse hole left behind");
});
...
test("regression: path writes are still immutable at every level", () => {
  ...
  assertEquals(original.list, [1, 2, 3], "the caller's array is untouched");
});
...
test("regression: subscribers still fire after store.clear()", () => {
  ...
  store.clear();
  store.set("s6", 2);
  assertEquals(seen, [1, undefined, 2], "clear notifies, then stays live");
  off();
  store.set("s6", 3);
  assertEquals(seen.length, 3, "unsubscribe still works");
});
...
test("regression: clear() drops entries that nobody is subscribed to", () => {
  ...
  store.set("s7", "value");
  store.clear();
  assertEquals(store.get("s7"), undefined);
});