...
import { test, assertEquals, assert, assertThrows } from "runtime:test";
...
test("props: object and function bindings are set before the child's setup runs", async () => {
  ...
  assertEquals(atSetup.data, { n: 1 }, "the object was there at setup");
  assertEquals(atSetup.save, "function", "so was the callback");
});
...
test("props: a callback binding can be invoked by the child", async () => {
  ...
  assertEquals(got, ["from the child"]);
});
...
test("props: replacing the object re-renders the child, every time", async () => {
  ...
  for (const n of [2, 3]) {
    ...
    await tick();
    await tick();
    assert(updRenders > before, `child re-rendered for n=${n}`);
    assertEquals(el.textContent, String(n), `child saw n=${n}`);
  }
});
...
test("props: handing back the same object does not re-render the child", async () => {
  ...
  assertEquals(el.querySelector("span").textContent, "b", "the parent updated");
  assertEquals(sameRenders, before, "but the child was left alone");
});
...
test("props: an object binding never lands in props as [object Object]", async () => {
  ...
  assertEquals(cleanProps.name, "ada", "string attributes still arrive");
  assert(
    !("data" in cleanProps),
    `props must not carry the object, got: ${JSON.stringify(cleanProps)}`,
  );
  for (const v of Object.values(cleanProps))
    assertEquals(typeof v, "string", "props is a string map");
});
...
test("props: string bindings still update the child as before", async () => {
  ...
  assertEquals(el.textContent, "one");
...
test("props: a tag name cannot be interpolated", async () => {
  // Each template is parsed once, so a ${...} in tag position is never a tag.
  // esdev's HTML parser rejects the placeholder outright — element names must
  // start with a lowercase ASCII letter — rather than silently rendering text,
  // so the template call itself throws and no element is ever created. Still
  // the first thing someone generating code reaches for, and it should be
  // pinned as loudly as it now fails.
  assertThrows(() => {
    html`<${"rp-str"}></${"rp-str"}>`;
  });
  define("rp-taginterp", () => () => html`<${"rp-str"}></${"rp-str"}>`);
  const el = await mountHost("rp-taginterp");
  assertEquals(el.querySelector("rp-str"), null);
});
...
test("props: calling the callback runs the newest closure", async () => {
  freshChild.save();
  assertEquals(freshCalls, [3], "the latest render's closure, not a stale one");
});