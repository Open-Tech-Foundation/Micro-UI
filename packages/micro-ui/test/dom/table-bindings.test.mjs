...
test("rows interpolated into <tbody> land inside it", async () => {
  ...
  assertEquals(tbody.querySelectorAll("tr").length, 2);
  // Nothing was fostered out to sit beside the table.
  assertEquals(el.querySelectorAll("table > tr").length, 0);
  assertEquals(
...
test("a row's cells interpolate into <tr>", async () => {
  ...
  assertEquals(tr.children.length, 3);
  assertEquals([...tr.children].every((c) => c.tagName === "TD"), true);
});
...
test("rows interpolated straight into <table>", async () => {
  ...
  assertEquals(el.querySelectorAll("table tr").length, 2);
  assertEquals(el.querySelector("table").previousSibling, null);
});
...
test("a keyed tbody reorders in place", async () => {
  ...
  assert(tbody.querySelectorAll("tr")[1] === first);
});
...
test("a conditional inside <tbody> adds and removes rows", async () => {
  ...
  const tbody = el.querySelector("tbody");
  assertEquals(tbody.querySelectorAll("tr").length, 1);

  state.show = true;
  await rerender();
  assertEquals(tbody.querySelectorAll("tr").length, 2);
  assertEquals(tbody.querySelectorAll("td")[1].textContent, "sometimes");
...
test("text and attributes inside a table cell still bind", async () => {
  ...
  assertEquals(td.className, "is-up");
  assertEquals(td.textContent, "$126.72");

  state.cls = "is-down";
  state.text = "-$41.93";
  await rerender();
  assert(el.querySelector("td") === td, "the cell was patched, not replaced");
  assertEquals(td.className, "is-down");
  assertEquals(td.textContent, "-$41.93");
});
...
test("<colgroup> and <tfoot> hold their bindings too", async () => {
  ...
  assertEquals(el.querySelectorAll("colgroup col").length, 2);
  assertEquals(el.querySelector("tfoot td").textContent, "total");
});
...
test("a binding inside <textarea> is still text, not a stray comment", async () => {
  ...
  assertEquals(ta.textContent, "hello");
  assertEquals(ta.textContent.includes("<!--"), false);

  state.value = "goodbye";
  await rerender();
  assertEquals(ta.textContent, "goodbye");
});
...
test("a binding inside <title> and <style> is still text", async () => {
  ...
  assertEquals(el.querySelector("title").textContent, "a title");
  assertEquals(el.querySelector("style").textContent, ".x { color: red }");
});
...
test("<select> keeps its interpolated options", async () => {
  ...
  assertEquals(el.querySelectorAll("select option").length, 2);
  assertEquals(el.querySelector("option").value, "ada");
});
...
test("an author's comment does not swallow a following binding", async () => {
  ...
  assertEquals(el.querySelector("span").textContent, "after");
});
...
test("markers in ordinary text, attributes and events are unaffected", async () => {
  ...
  assertEquals(div.getAttribute("data-x"), "y");
  expect(div.textContent.replace(/\s+/g, " ")).toMatch(/one two three four/);
