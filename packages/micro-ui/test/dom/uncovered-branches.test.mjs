...
test("branches: a render returning a bare text vnode mounts it", async () => {
  ...
  assertEquals(el.firstChild.nodeType, 3, "a real text node, not an element");
});
...
test("branches: a bare text vnode re-renders in place", async () => {
  ...
  rerender();
  await tick();
  assertEquals(el.textContent, "n=1");
  assert(el.firstChild === node, "patched, not replaced");
});
...
test("branches: an element moved into SVG keeps its handler and text", async () => {
  ...
  assertEquals(el.querySelector("a").namespaceURI, "http://www.w3.org/1999/xhtml");
  ...
  assertEquals(a.namespaceURI, "http://www.w3.org/2000/svg", "rebuilt in SVG");
  assertEquals(a.textContent, "label", "its text came across");
  a.dispatchEvent(new window.Event("click", { bubbles: true }));
  assertEquals(hits, ["hit"], "and so did its handler");
});
...
test("branches: a handler object can be swapped for a function", async () => {
  ...
  assertEquals(seen, ["object", "function"]);
});
...
test("branches: an object bound to an SVG attribute is stringified", async () => {
  ...
  assertEquals(el.querySelector("path").getAttribute("d"), "M0 0 L10 10");
});
...
test("branches: a prop whose binding goes nullish is deleted", async () => {
  ...
  document.body.appendChild(el);
  await tick();
  assertEquals(el.textContent, "here");

  el.removeAttribute("label");
  update(el);
  await tick();
  assertEquals(el.textContent, "none", "the prop was dropped, not stale");
  void label;
  void ref;
});
...
test("branches: a nullish binding removes the child's prop", async () => {
  ...
  await tick();
  await tick();
  assertEquals(el.textContent, "-", "nullish binding cleared the prop");
  assertEquals(seen.at(-1), false, "and the key is gone, not undefined");
});
...
test("branches: an attribute dropped from the template removes the prop", async () => {
  ...
  await tick();
  await tick();
  assertEquals(el.textContent, "-", "the removed attribute removed the prop");
});
...
test("branches: an object binding replaced by a string swaps channels", async () => {
  ...
  await tick();
  await tick();
  assertEquals(el.textContent, "obj:1");

  data = "plain";
  update(ref);
  await tick();
  await tick();
  assertEquals(el.textContent, "plain", "props now carries it");
});
...
test("branches: a reused vnode is rebuilt when it lands in a new namespace", async () => {
  ...
  const a = el.querySelector("a");
  assert(a, "the reused node is in the tree");
  assertEquals(a.namespaceURI, "http://www.w3.org/2000/svg", "recreated in SVG");
  assertEquals(a.textContent, "label", "its text survived the rebuild");
  a.dispatchEvent(new window.Event("click", { bubbles: true }));
  assertEquals(hits, ["hit"], "so did its handler");
});
...
test("branches: a string prop replaced by an object leaves props", async () => {
  ...
  await tick();
  await tick();
  assertEquals(el.textContent, "str:plain");

  data = { n: 9 };
  update(ref);
  await tick();
  await tick();
  assertEquals(el.textContent, "obj:9", "props entry removed, property set");
});