...
test("a javascript: href is refused and reported", async () => {
  ...
  assertEquals(el.querySelector("a").hasAttribute("href"), false);
...
test("every navigable attribute is covered", async () => {
  ...
  assertEquals(el.querySelector("img").hasAttribute("src"), false);
  assertEquals(el.querySelector("form").hasAttribute("action"), false);
  assertEquals(el.querySelector("button").hasAttribute("formaction"), false);
  assertEquals(el.querySelector("iframe").hasAttribute("srcdoc"), false);
});
...
test("ordinary URLs are untouched", async () => {
  ...
  for (const url of fine) {
    ...
    assertEquals(logged, [], `warned about: ${url}`);
  }
});
...
test("a href that turns dangerous on a later render is removed", async () => {
  ...
  assertEquals(el.querySelector("a").getAttribute("href"), "/safe");
...
test("an svg xlink:href is covered", async () => {
  ...
  assertEquals(el.querySelector("a").getAttribute("xlink:href"), null);
});