...
const { define, html, update } = await import(
  `../../src/index.ts?nested-fuzz-${Date.now()}`
);
    ...
    for (let step = 0; step < 90; step++) {
      ...
      for (const item of model) {
        if (beforeOuter.has(item.key))
          assert.equal(actualOuter.get(item.key), beforeOuter.get(item.key));
        if (beforeKinds.get(item.key) !== item.kind) continue;
        for (const child of item.children) {
          if (beforeChildren.has(child.key))
            assert.equal(
              actualChildren.get(child.key),
              beforeChildren.get(child.key),
